...
class DeeperDAGNN_node_Virtualnode(torch.nn.Module):
    def __init__(self, atom_dim, edge_dim, num_layers, emb_dim,
                 drop_ratio=0.5, JK="last", aggr='softmax', norm='batch'):
        '''
        ...
        self.atom_encoder = torch.nn.Linear(atom_dim, emb_dim)
        ...
        self.linear2 = torch.nn.Linear(emb_dim, 1)
        ...
        for i in range(1, num_layers + 1):
            conv = GENConv(emb_dim, emb_dim, aggr=aggr, t=1.0, learn_t=True,
                           learn_p=True, num_layers=2, norm=norm)
            if norm=="batch":
        ...
        for layer in range(num_layers - 1):
            if norm=="batch":
                ...
                self.mlp_virtualnode_list.append(torch.nn.Sequential(
                    torch.nn.Linear(emb_dim, emb_dim),
                    torch.nn.LayerNorm(emb_dim, elementwise_affine=True),
                    torch.nn.ReLU(),
                    torch.nn.Linear(emb_dim, emb_dim),
                    torch.nn.LayerNorm(emb_dim, elementwise_affine=True),
                    torch.nn.ReLU()
    ...
    def forward(self, batched_data, pairs):
        ...
        pdists = node_representation[pairs].view(len(pairs), 2*self.emb_dim)

        # linears
        pdists = self.linear1(pdists)
        pdists = self.activation(pdists)
        pdists = self.linear2(pdists)
