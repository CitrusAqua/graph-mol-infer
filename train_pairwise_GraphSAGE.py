...
def gen_pairs(batch):
    ...
    x_indices = np.arange(len(batch))
    pairs = []
    batch_np = batch.numpy()
    for i in range(num_batches):
        sub_indices = torch.Tensor(x_indices[batch_np == i])
        combs = torch.combinations(sub_indices, r=2)
        pairs.append(combs)
    pairs = torch.cat(pairs).long()
    return pairs
...
================================================================================
'''
from models.GraphSAGE_pairwise import GraphSAGE_Pairwise
from torch.optim import lr_scheduler

print('-----------------')
print('Building model.')

model = GraphSAGE_Pairwise(
